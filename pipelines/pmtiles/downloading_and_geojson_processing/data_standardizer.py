...
class DataStandardizer:
    ...
    def standardize_ownership(self, county_data, county_name):
        ...
        for i, feature in enumerate(tqdm(county_data["features"], desc="Standardizing features"), start=1):
            ...
            # Extract county_parcel_id first to use in GFI
            county_parcel_id = self._extract_from_mapping(props, mappings, "parcel_id")
            
            # Generate GFI as county + county_parcel_id (deterministic across runs)
            # If county_parcel_id is missing, fall back to sequential number with unknown marker
            county_normalized = county_name.lower()
            if county_parcel_id:
                unique_id = f"{county_normalized}_{county_parcel_id}"
            else:
                # Fallback for missing parcel_id (should be rare)
                unique_id = f"{county_normalized}_unknown_{str(i).zfill(6)}"
            ...
            # Note: county and county_parcel_id removed since they're encoded in GFI
            standardized_props = {
                "GFI": unique_id,
    ...
    def save_standardized_data(self, standardized_data, county_name):
        ...
        from pathlib import Path
        tile_cycle_dir = Path(__file__).parent.parent
        output_path = tile_cycle_dir / "final_parcels" / f"{county_name}_data_files" / f"{county_name}_final_ownership.geojson"
