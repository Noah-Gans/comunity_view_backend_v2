...
class TetonPropertyDetailsScraper:
    ...
    def scrape(self) -> dict:
        ...
        # Extract additional links from Layer 0 API response
        additional_links = self.extract_links_from_layer_0(layer_0_data)
        if additional_links:
            result['_additional_links'] = additional_links
    ...
    def map_to_canonical(self, layer_0_data: dict, layer_2_data: dict, layer_3_data: dict) -> dict:
        ...
        if layer_0_data.get('features'):
            ...
            # Map zoning
            result['zoning'] = parcel_data.get('zoning', '')
            
            # Map ldr_plan
            result['ldr_plan'] = parcel_data.get('ldr_plan', '')
    ...
    def extract_links_from_layer_0(self, layer_0_data: dict) -> dict:
        """Extract additional links (map_no, deed_no, smart_gov, ldr_plan) from Layer 0 API response."""
        links = {}
        
        if layer_0_data.get('features'):
            parcel_data = layer_0_data['features'][0]['attributes']
            
            # Extract the three additional links that aren't in construct_links()
            if parcel_data.get('map_no'):
                links['map_no'] = parcel_data.get('map_no')
            if parcel_data.get('deed_no'):
                links['deed_no'] = parcel_data.get('deed_no')
            if parcel_data.get('smart_gov'):
                links['smart_gov'] = parcel_data.get('smart_gov')
            if parcel_data.get('ldr_plan'):
                links['ldr_plan'] = parcel_data.get('ldr_plan')
        
        logger.debug(f"(Teton County WY Property) Extracted additional links: {list(links.keys())}")
        return links
