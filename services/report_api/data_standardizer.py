...
class DataStandardizer:
    ...
    def standardize_property_data(self,) -> Dict:
        ...
        # Get developments first to calculate aggregates
        developments = self.extract_field("developments", data_sources)
        
        # Calculate aggregate fields from developments
        total_sq_ft = self._calculate_total_sq_ft(developments)
        largest_structure_sq_ft = self._find_largest_structure_sq_ft(developments)
        ...
        standardized = {
            ...
            "data": {
                ...
                "zoning": self.extract_field("zoning", data_sources),
                # Aggregate fields
                "total_sq_ft": total_sq_ft,
                "largest_structure_sq_ft": largest_structure_sq_ft,
        ...
        # For Lincoln County, clean the mailing address (remove % ownership)
        if self.county == "lincoln_county_wy" and standardized["data"].get("mailing_address"):
            import re
            mailing_address = standardized["data"]["mailing_address"]
            if isinstance(mailing_address, str):
                # Remove trailing percentage (e.g., "100%")
                standardized["data"]["mailing_address"] = re.sub(r"\s+\d+(?:\.\d+)?%$", "", mailing_address).strip()
    ...
    def _create_general_info(self) -> Dict:
        ...
        # For Lincoln County, clean the mailing address (remove % ownership)
        if self.county == "lincoln_county_wy" and general_info.get("mailing_address"):
            import re
            mailing_address = general_info["mailing_address"]
            if isinstance(mailing_address, str):
                # Remove trailing percentage (e.g., "100%")
                general_info["mailing_address"] = re.sub(r"\s+\d+(?:\.\d+)?%$", "", mailing_address).strip()
    ...
    def _calculate_total_sq_ft(self, developments: List[Dict]) -> Optional[float]:
        """Calculate total square footage across all developments."""
        if not developments:
            return None
        ...
        for dev in developments:
            ...
            if sq_ft:
                try:
                    ...
                    sq_ft_value = float(sq_ft)
                    total += sq_ft_value
                except (ValueError, TypeError):
                    logger.debug(f"Could not convert sq_ft to float: {sq_ft}")
                    continue
        
        return total if total > 0 else None
    
    def _find_largest_structure_sq_ft(self, developments: List[Dict]) -> Optional[float]:
        """Find the largest structure by square footage."""
        if not developments:
            return None
