...
def create_search_index():
    ...
    # Base path for GeoJSON files - navigate to pipelines/pmtiles from search_api
    script_dir = Path(__file__).parent
    project_root = script_dir.parent.parent  # Go up to project root
    base_path = project_root / "pipelines" / "pmtiles" / "final_parcels"
        ...
        try:
            ...
            for feature in county_data.get('features', []):
                try:
                    ...
                    # Parse county_parcel_id from GFI (format: county_name_county_parcel_id)
                    gfi = properties.get("GFI", "")
                    county_parcel_id = ""
                    if gfi:
                        # GFI format: {county_name}_{county_parcel_id}
                        # Since we know the county name, we can extract the parcel_id by removing the county prefix
                        county_prefix = f"{county}_"
                        if gfi.startswith(county_prefix):
                            county_parcel_id = gfi[len(county_prefix):]
                            # If it starts with "unknown_", that's our fallback format (no real parcel_id)
                            if county_parcel_id.startswith("unknown_"):
                                county_parcel_id = ""  # Empty for unknown cases
